...
export function TypeORMAdapter(dataSource, options) {
    ...
    return {
        ...
        async getUserByAccount(provider_providerAccountId) {
            const m = await getManager(c);
            // @ts-expect-error
            const account = await m.findOne(AccountEntityName, 
            // @ts-expect-error
        ...
        async unlinkAccount(providerAccountId) {
            const m = await getManager(c);
            await m.delete(AccountEntityName, providerAccountId);
        },
        ...
        async getSessionAndUser(sessionToken) {
            const m = await getManager(c);
            const sessionAndUser = await m.findOne(SessionEntityName, { where: { sessionToken }, relations: ["user"] });
            if (!sessionAndUser)
                return null;
